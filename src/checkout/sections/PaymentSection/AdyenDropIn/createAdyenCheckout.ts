...
export function handlePaymentResult(
	...
	switch (result.payment.resultCode) {
		...
		case AdyenApiPaymentResponse.ResultCodeEnum.Success: {
			...
			const domain = new URL(holipolyApiUrl).hostname;
			const newUrl = replaceUrl({
				query: {
					...
					holipolyApiUrl,
					// @todo remove `domain`
					// https://github.com/holipoly/holipoly-dashboard/issues/2387
					// https://github.com/holipoly/holipoly-app-sdk/issues/87
					domain,
				},
			});
