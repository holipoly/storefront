...
export async function Footer({ channel }: { channel: string }) {
	...
	const channels = process.env.HOLIPOLY_APP_TOKEN
		? await executeGraphQL(ChannelsListDocument, {
				withAuth: false, // disable cookie-based auth for this call
				headers: {
					// and use app token instead
					Authorization: `Bearer ${process.env.HOLIPOLY_APP_TOKEN}`,
				},
			})
		: null;
			...
			<div className="mx-auto max-w-7xl px-4 lg:px-8">
				...
				<div className="flex flex-col justify-between border-t border-neutral-200 py-10 sm:flex-row">
					...
					<p className="flex gap-1 text-sm text-neutral-500">
						Powered by{" "}
						<Link target={"_blank"} href={"https://holipoly.com/"}>
							Holipoly
						</Link>{" "}
						<Link href={"https://github.com/holipoly/holipoly"} target={"_blank"} className={"opacity-30"}>
							<Image alt="Holipoly github repository" height={20} width={20} src={"/github-mark.svg"} />
						</Link>
					</p>
				</div>
			</div>
